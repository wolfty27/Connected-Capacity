...
const TopBar = ({ onMenuClick, isSidebarOpen }) => {
    ...
    const getPageTitle = () => {
        ...
        if (path.includes('/metrics/tfs')) return 'Time-to-First-Service';
        if (path.includes('/workforce/capacity')) return 'Capacity Dashboard';
        if (path.includes('/scheduling')) return 'Staff Scheduling';
        return 'Overview';
    };
        ...
        <header className="h-16 bg-white border-b border-slate-200 sticky top-0 z-30 px-4 md:px-8 flex items-center justify-between shadow-sm">
            <div className="flex items-center gap-4">
                ...
                <div className="flex items-center gap-2">
                    ...
                    <span className="font-bold text-lg tracking-tight text-teal-900 hidden sm:inline">Connected Capacity</span>
                </div>

                {/* Page Title */}
                <div className="hidden md:flex items-center gap-2 ml-4 pl-4 border-l border-slate-200">
                    <h2 className="text-sm font-semibold text-slate-600 truncate max-w-[300px]">{getPageTitle()}</h2>
                </div>
            </div>
