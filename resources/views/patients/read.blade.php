                    ...
                    <div class="col-12">
                        <div class="page-title-box">
                            ...
                            @if(auth()->user()->role == 'admin')
                                <h4 class="page-title">Available Patients</h4>
                            @else
                                    ...
                                    @foreach($data as $record)
                                        <tr>
                                            ...
                                            <td class="table-action">
                                                ...
                                                @if(auth()->user()->role == 'hospital' && $record['status'] != 'Placement Made')
...
{{--                                                check if $record['calendly'] exists, if not, return an error msg--}}
                                                @if(auth()->user()->role == 'retirement-home' && $record['status'] == 'Available')
                                                    @if ($record['calendly'])
                                                        <a href="" class="action-icon patientID"  data-bs-toggle="tooltip" data-bs-placement="top"
                                                           data-bs-title="Book Appointment" id={{$record['id']}} onclick="Calendly.initPopupWidget({url: '{{config('calendly.calendly_base_url')}}{{ $record['calendly'] }}?hide_gdpr_banner=1'});return false;"> <i class="mdi mdi-account-check-outline text-info"></i></a>
                                                    @else
