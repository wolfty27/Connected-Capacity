...
class ScenarioGenerator implements ScenarioGeneratorInterface
{
    ...
    /**
     * Add services specific to an axis that wouldn't be generated by algorithms alone.
     * This ensures each axis produces differentiated bundles.
     */
    protected function addAxisSpecificServices(array $services, ScenarioAxis $axis, PatientNeedsProfile $profile): array
    {
        // Define which extra services each axis should include
        $axisServices = match ($axis) {
            ScenarioAxis::COMMUNITY_INTEGRATED => [
                'ADP' => ['frequency' => 2, 'duration' => 240, 'priority' => 'core'],      // Adult Day Program
                'MEAL' => ['frequency' => 5, 'duration' => 15, 'priority' => 'recommended'], // Meal delivery
                'TRANS' => ['frequency' => 2, 'duration' => 60, 'priority' => 'recommended'], // Transportation
            ],
            ScenarioAxis::CAREGIVER_RELIEF => [
                'RES' => ['frequency' => 2, 'duration' => 240, 'priority' => 'core'],      // Respite Care
                'HMK' => ['frequency' => 2, 'duration' => 120, 'priority' => 'core'],      // Homemaking
                'CGC' => ['frequency' => 1, 'duration' => 60, 'priority' => 'recommended'], // Caregiver coaching
            ],
            ScenarioAxis::SAFETY_STABILITY => [
                'RPM' => ['frequency' => 7, 'duration' => 15, 'priority' => 'core'],       // Remote monitoring
                'PERS' => ['frequency' => 7, 'duration' => 5, 'priority' => 'core'],       // PERS/Lifeline
                'SEC' => ['frequency' => 7, 'duration' => 15, 'priority' => 'recommended'], // Security checks
            ],
            ScenarioAxis::TECH_ENABLED => [
                'RPM' => ['frequency' => 7, 'duration' => 15, 'priority' => 'core'],       // Remote monitoring
                'TELE' => ['frequency' => 2, 'duration' => 30, 'priority' => 'core'],      // Telehealth
                'VPC' => ['frequency' => 1, 'duration' => 20, 'priority' => 'recommended'], // Virtual primary care
            ],
            ScenarioAxis::COGNITIVE_SUPPORT => [
                'DEM' => ['frequency' => 3, 'duration' => 120, 'priority' => 'core'],      // Dementia care
                'BEH' => ['frequency' => 2, 'duration' => 90, 'priority' => 'core'],       // Behavioural supports
                'ADP' => ['frequency' => 2, 'duration' => 240, 'priority' => 'recommended'], // Adult day program
            ],
            ScenarioAxis::RECOVERY_REHAB => [
                // Therapy is already handled by algorithms, just boost
            ],
            ScenarioAxis::MEDICAL_INTENSIVE => [
                'DEL-ACTS' => ['frequency' => 5, 'duration' => 45, 'priority' => 'core'],  // Delegated acts
            ],
            default => [],
        };
        ...
        // Add axis-specific services that don't already exist
        foreach ($axisServices as $code => $config) {
            ...
            $services[] = [
                ...
                'clinical_rationale' => "Added for {$axis->getLabel()} focus",
            ];
        }
